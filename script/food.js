const appendData =(data,location)=>{
    ...
    data.map((ele)=>{
                    ...
                    ${foodItems.map(item => `
                        <div class="food-item-card">
                            ...
                            <div class="food-item-info">
                                ...
                                <button class="add-food-item-btn" onclick="addFoodItemToCart('${item.id}', '${item.name}', ${item.price}, '${item.image}', '${ele.place}')">
                                    Add to Cart
                                </button>
                            </div>
                        </div>
        ...
        subHeadDEle1.innerText=ele.place;
        subHeadDEle1.setAttribute("class","place");
        subHeadDEle2.innerText=ele.food;
        subHeadDEle2.setAttribute("class","food");
        subHeadDEle3.innerText=`₹${ele.price} for one`;
        subHeadDEle3.setAttribute("class","price");
        subHeadDEle4.innerText=ele.rating;
        subHeadDEle4.setAttribute("class","rating");
        subHeadDEle5.src="https://cdn-icons-png.flaticon.com/128/1828/1828884.png";
        subHeadDEle5.setAttribute("class","ratingImg");
        subHeadDEle6.innerText=ele.time;
        subHeadDEle6.setAttribute("class","time");
        ...
        mainDiv.append(addToCartDiv,img,time,headDiv1,headDiv2,foodItemsDiv);
        ...
        mainDiv.setAttribute("class","containerDiv");
        location.append(mainDiv);
    })

}

// Generate food items for each restaurant
const generateFoodItems = (restaurant) => {
    ...
    else if (restaurant.food && restaurant.food.toLowerCase().includes('pizza')) category = 'Pizza';
    else if (restaurant.food && restaurant.food.toLowerCase().includes('burger')) category = 'Burger';
    ...
    return items.map((item, index) => ({
        ...
        restaurantName: restaurant.place || restaurant.name
    }));
};
...
let addToCart=(ele)=>{
    let cartProducts = JSON.parse(localStorage.getItem("cartProducts")) || [];
    let existingProduct = cartProducts.find(product => product.id === ele.id);
    
    if (existingProduct) {
        existingProduct.qty = (existingProduct.qty || 1) + 1;
        existingProduct.price = (ele.price * existingProduct.qty);
    } else {
        ele.qty = 1;
    ...
    alert.style.backgroundColor = "green";
    alert.style.border = "1px solid green";
    alert.innerHTML = `${ele.food} Added To Cart <i class="fa-sharp fa-solid fa-circle-check"></i>`;
    alert.style.display = "block";
...
const checkLogin=()=>{
    ...
    if(login==null){
        ...
        setTimeout(() => {
            ...
            document.querySelector("#ordersSection").addEventListener("click",()=>{
